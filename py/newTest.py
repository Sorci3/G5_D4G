...
print(f"Nombre d'exemples d'entraînement: {len(dataset)}")
print(f"Nombre d'exemples de validation: {len(eval_dataset)}")

# Fonction de préparation des données
def preprocess_function(examples):
    """
    Formate les données au format instruction-following
    """
    inputs = []
    for document, summary in zip(examples["document"], examples["summary"]):
        # Format prompt clair pour le résumé court
        prompt = f"""### Instruction:
Résume le texte suivant en {TARGET_SUMMARY_LENGTH} maximum.
...
tokenized_dataset = dataset.map(
    ...
    desc="Tokenization du dataset d'entraînement"
)
...
def generate_summary(text, max_new_tokens=20):
    """
    Génère un résumé court à partir d'un texte
    """
    prompt = f"""### Instruction:
Résume le texte suivant en {TARGET_SUMMARY_LENGTH} maximum.
...
"""
    ...
    # Extraire uniquement le résumé généré
